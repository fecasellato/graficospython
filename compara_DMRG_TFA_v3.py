...
def carregar_tfa_outputs(padrao="tfa_output_Nup*.txt"):
    ...
    A polarização média é extraída do nome do arquivo.
    """
    dens_tfa = {}
    mag_tfa = {}
    ...
    for nome_arq in arquivos:
        # Extrair Nup e Ndown do nome do arquivo usando regex
        # Exemplo: tfa_output_Nup49_Ndown1.txt
        match = re.search(r'Nup(\d+)_Ndown(\d+)', nome_arq)
        if not match:
            print(f"[AVISO] Nome de arquivo inesperado: {nome_arq}. Pulando.")
            continue
        Nup = int(match.group(1))
        Ndown = int(match.group(2))
        # Detectar o número de sítios automaticamente
        with open(nome_arq, "r") as f:
            linhas = [linha for linha in f if not linha.startswith("#")]
        Ns = len(linhas)  # Número de sítios, ignorando linhas de comentário
        ...
        if dens and mag and len(dens) == Ns:
            polarizacao_media = round((Nup - Ndown) / Ns, 4) # melhor precisão
            dens_tfa[polarizacao_media] = dens
            mag_tfa[polarizacao_media] = mag
...
if __name__ == "__main__":
    ...
    for pol in polarizacoes_comuns:
        ...
        if len(n_tfa) != len(n_dmrg) or len(m_tfa) != len(m_dmrg):
            print(f"[PULANDO] Tamanho incompatível para pol={pol}: n_tfa={len(n_tfa)}, n_dmrg={len(n_dmrg)}, m_tfa={len(m_tfa)}, m_dmrg={len(m_dmrg)}")
            continue

        # Criar índice de sítio compatível (1 a 100)
        site_ids = list(range(1, len(n_tfa) + 1))  # tanto para TFA (i+1) quanto DMRG (começa do 1)
